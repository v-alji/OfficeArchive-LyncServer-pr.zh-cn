...
控制器池的防火墙端口要求由用于从边缘服务器的内部接口或反向代理的内部接口内部接口中的控制器建立通信的端口。 默认情况下，Microsoft Lync Server 2013 需要从反向代理（以及前端池和前端服务器）打开端口 HTTP/TCP 8080 和 HTTPS/TCP 4443。 此外，必须存在) 从边缘服务器内部接口到 Director 以及前端池和前端服务器的会话初始协议 (SIP 的通信。 SIP 协议使用 SIP/MTLS/TCP 5061，从边缘服务器到前端池和前端服务器。 还必须创建允许从 Director、前端池和前端服务器到边缘服务器内部接口的 SIP/MTLS/TCP 5061 通信的规则。
...
<td><p>边缘服务器内部接口</p></td>
<td><p>控制器硬件负载平衡器 VIP</p></td>
<td><p>从边缘服务器到控制器和前端服务器的 SIP 通信。</p></td>
</tr>
<tr class="odd">
