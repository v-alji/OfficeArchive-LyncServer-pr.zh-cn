...
6.  在 " **编辑设备配置**" 下的 " **电话锁定**" 下，选中或清除 " **强制设备锁定** " 复选框，默认情况下 (选中 "默认情况下) "，并指定默认的最小 PIN 长度 (默认) 和超时期限 (10 分钟) 。 我们建议使用这些默认值或增加 PIN 长度和/或减少超时时间。
...
## <a name="to-modify-the-sip-security-mode"></a>修改 SIP 安全模式
  ...
  - 此命令将 UC 电话设置的全局集合的 SIPSecurityMode 设置为 "中"。 SIP 安全性还可以设置为 "低" 或 "高" (默认值) 。
