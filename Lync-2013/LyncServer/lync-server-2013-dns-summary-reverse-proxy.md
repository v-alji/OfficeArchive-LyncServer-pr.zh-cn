...
## <a name="reverse-proxy-network-adapter-requirements"></a>反向代理网络适配器要求
  ...
  - **网络适配器 1 (内部接口)** 示例
    ...
    确保存在从网络到包含 Lync Server 前端池服务器的反向代理内部接口的路由， (例如从172.25.33.0 到 192.168.10.0) 。

  - **网络适配器 2 (外部接口)** 示例
    
    至少有一个公共 IP 地址分配给此网络适配器。
    
    网关定义为指向外部外围设备中的路由器或集成防火墙。  (方案示例中的 10.45.16.1) 
...
> [!IMPORTANT]  
> 定义和发布控制器的 DNS 记录既不是前端池，也不是控制器决策。 如果您使用的是董事，则必须定义和发布 Director 和前端池外部 web 服务。 如果在拓扑中定义了身份验证和其他使用) 的特定流量类型 (将首先发送给 Director。
