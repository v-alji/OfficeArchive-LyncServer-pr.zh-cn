...
方案定义在集中日志记录服务中使用的范围 (，即全局、网站、池或计算机) 以及要使用的提供程序。 通过使用方案，可以启用或禁用对提供程序进行的跟踪（例如，S4、SIPStack、IM 和 Presence）。 通过配置方案，可将针对给定逻辑集合的、满足某个特定问题条件的所有提供程序组合在一起。 如果你发现需要修改方案以满足你的疑难解答和日志记录需求，Lync Server 2013 调试工具将向你提供名为 *ClsController* 的 Windows PowerShell 模块，该模块包含名为 *Edit-CsClsScenario* 的函数。 此模块的用途是编辑命名的方案的属性。 本主题提供了此模块的工作方式的示例。 Lync Server 2013 调试工具从以下链接下载： [https://go.microsoft.com/fwlink/?LinkId=285257](https://go.microsoft.com/fwlink/?linkid=285257)
...
若要使用 Lync Server 管理外壳程序运行集中式日志记录服务功能，您必须是 CsAdministrator 或 CsServerAdministrator 基于角色的访问控制的成员 (RBAC) 安全组或包含这两个组之一的自定义 RBAC 角色。 若要返回此 cmdlet 已分配到的所有 RBAC 角色的列表，包括你自己创建的任何自定义 RBAC 角色，请从 Lync Server Management Shell 或 Windows PowerShell 提示中运行以下命令：
...
本主题的其余部分重点说明了如何定义方案、修改方案、检索正在运行的方案、删除方案以及指定方案为优化故障排除而包含的内容。 有两种方法可以发出集中式日志记录服务命令。 默认情况下，你可以使用 CLSAgent 中的 "目录 C：程序文件" 中所示的 CLSController.exe " \\ \\ \\ Microsoft Lync Server 2013 \\ "。 或者，你可以使用 Lync Server Management Shell 颁发 Windows PowerShell 命令。 重要的区别在于，在命令行中使用 CLSController.exe 时，有有限的可用方案选择。 使用 Windows PowerShell 时，可以定义要在日志记录会话中使用的新方案。

如 [Lync Server 2013 中的集中式日志记录服务概述](lync-server-2013-overview-of-the-centralized-logging-service.md)，方案的元素如下所述：

  - **提供商**   如果你熟悉 OCSLogger，则提供的组件是你选择的组件，用于告诉 OCSLogger 跟踪引擎应从哪些内容收集日志。 提供程序是一些相同的组件，在许多情况下，它们的名称与 OCSLogger 中组件的名称相同。 如果您不熟悉 OCSLogger，则提供商是集中式日志记录服务可以从中收集日志的服务器角色特定组件。 有关提供程序配置的详细信息，请参阅 [在 Lync Server 2013 中配置集中式日志记录服务的提供商](lync-server-2013-configuring-providers-for-centralized-logging-service.md)。
...
2.  每个范围只能有两个方案。 但是，你不受限于一组提供程序的数量限制。 在此示例中，假设我们创建了三个提供程序，并且你希望将所有三个提供程序分配给你定义的方案。 提供程序变量名称为 LyssProvider、ABServerProvider 和 SIPStackProvider。 若要定义多个提供程序并将其分配给某个方案，请在 Lync Server 命令行管理程序或 Windows PowerShell 命令提示符处键入以下内容：
    ...
    > [!NOTE]  
    > 正如在 Windows PowerShell 中已知的，用于创建使用的值的哈希表的约定 <CODE>@{&lt;variable&gt;=&lt;value1&gt;, &lt;value2&gt;, &lt;value&gt;...}</CODE> 称为 <EM>splatting</EM>。 有关 Windows PowerShell 中的 splatting 的详细信息，请参阅 <A href="https://go.microsoft.com/fwlink/p/?linkid=267760">https://go.microsoft.com/fwlink/p/?LinkId=267760</A> 。
...
## <a name="to-load-and-unload-the-edit-csclsscenario-cmdlet-using-the-clscontrollerpsm1-module"></a>若要使用 ClsController 模块加载和卸载 Edit-CsClsScenario cmdlet
...
2.  在 Windows PowerShell 中，键入：
    ...
    > [!TIP]  
    > 成功加载该模块后，将返回到 Windows PowerShell 命令提示符。 若要确认是否已加载模块且 Edit-CsClsScenario 可用，请键入 <CODE>Get-Help Edit-CsClsScenario</CODE> 。 您应看到 EditCsClsScenario 的语法的基本概要。
...
3.  若要卸载模块，请键入：
    ...
    > [!TIP]  
    > 成功卸载该模块后，将返回到 Windows PowerShell 命令提示符。 若要确认卸载了该模块，请键入 <CODE>Get-Help Edit-CsClsScenario</CODE> 。 Windows PowerShell 将尝试查找 cmdlet 的帮助并失败。
...
2.  若要将提供程序添加到 AlwaysOn 方案，请键入：
    ...
    \-Loglevel 可以是 "严重"、"错误"、"警告"、"详细信息"、"调试" 或 "全部" 类型。 -Flags 可以是提供程序支持的任何标志，如 TF \_ COMPONENT、tf \_ 诊断。 –Flags 的值可以为 ALL
