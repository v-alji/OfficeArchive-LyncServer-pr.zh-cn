...
2.  在 " **定义前端池 FQDN** " 页面上，输入要创建的池 (FQDN) 的完全限定的域名，然后单击 " **企业版前端池**"，然后单击 " **下一步**"。

3.  在 " **在此池中定义计算机** " 页面上，输入池中第一个前端服务器的计算机 FQDN，然后单击 " **添加**"。 对任何其他计算机重复此步骤， (最多 12) 要添加到池中的计算机，然后单击 " **下一步**"。

4.  在 " **选择功能** " 页面上，选中您希望在此前端池上的功能所对应的复选框。 例如，如果你仅部署即时消息 (IM) 和状态显示功能，则可以选择 " **会议** " 复选框以允许多方 IM，但不会选择 **拨入 (PSTN) 会议**、 **企业语音** 或 **呼叫许可控制** 复选框，因为它们代表语音、视频和协作会议功能。
      ...
      - **会议**   此选择支持一组丰富的功能，包括：
          ...
          - A/V 式会议，使用户可以在不需要外部服务（如 Live Meeting 服务或第三方音频桥）的情况下 (A/V) 会议中进行实时音频/视频。
    
      - **(PSTN) 会议的电话拨入式**   允许用户通过使用公共交换电话网络 (PSTN) 电话，无需音频会议提供商，即可加入 Lync Server 2013 会议的音频部分。
    
      - **企业语音**   企业语音是 Lync Server 2013 中的 IP 语音 (VoIP) 解决方案，允许用户拨打和接收电话呼叫。 如果您计划将 Lync Server 2013 用于语音呼叫、语音邮件和其他使用硬件设备或软件客户端的功能，则可以部署此功能。
      ...
      - **监控**   通过监视服务器，你可以收集用于描述你的网络和终结点上的媒体质量的数字数据、与 VoIP 呼叫、即时消息、A/V 对话、会议、应用程序共享和文件传输的相关信息，以及呼叫错误和针对失败呼叫的疑难解答信息。
    ...
    下表显示了 (top) 的可用功能以及向 (左) 的用户提供的函数。 表格中的选择是为您的组织启用这些功能应选择的内容。
...
9.  在 " **指定 Web 服务 URL** " 页面上，执行下列一项或两项操作：
    ...
    > [!IMPORTANT]
    > 基 URL 指的是 URL 的 Web 服务标识减去 https://。 例如，如果池的 Web 服务的完整 URL 是 https://pool01.contoso.net ，则基 URL 为 pool01.contoso.net。
    ...
    1.  如果你要配置 DNS 负载平衡，请选中 " **替代内部 Web 服务池 FQDN** " 复选框，输入内部基础 url (该 url 必须不同于池 FQDN，例如内部 \<your base URL\> **基础 url** 中的内部) 。
...
10. 如果您在 "**选择功能**" 页面上选择了 "**会议**"，请在 "**选择 office web apps 服务器**" 页面上选择 "**关联池与 Office web Apps 服务器**"，然后单击 "**新建** (或从下拉列表中选择现有的 Office Web Apps 服务器) 。
