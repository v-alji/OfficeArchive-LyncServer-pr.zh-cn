...
当您部署前端服务器时，Lync 服务器将为存档创建全局策略。 默认情况下，存档在全局策略中处于禁用状态。 全局策略控制是否为整个部署的内部和外部通信启用存档，除非您设置网站或用户策略（这将覆盖全局策略），或者您对部分或所有用户使用 Microsoft Exchange 集成。 如果您使用 Microsoft Exchange 集成，则全局策略不会应用于托管在 Exchange 2013 上的任何用户，并且邮箱置于 In-Place 保留状态。
