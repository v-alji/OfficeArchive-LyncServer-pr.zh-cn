...
7.  在 " **内部发布详细信息** " 页面上的 " **内部网站名称**" 中，键入你的控制器池 (FQDN) 的完全限定的域名 (例如，lyncdir01) 。 如果你要为前端池上的外部 Web 服务 URL 创建规则，请在前端池的前面键入硬件负载平衡器 (HLB) 的 VIP 地址。
