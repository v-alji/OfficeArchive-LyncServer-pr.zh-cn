...
您提供给服务提供商的信息取决于 SIP 中继连接类型：
  ...
  - 对于多协议标签切换 (MPLS) 或专用网络连接，请为 ITSP 提供你的外围网络中的路由器的公共可路由 IP 地址 (也称为 DMZ、隔离区域和屏蔽子网) 。 验证 ITSP 上的网关或会话边界控制器 (SBC) 是否可以联系到此地址。 还为 ITSP 提供中介服务器的 FQDN。
...
1.  如果 ITSP 支持仅 (TCP) 传输控制协议，则不需要证书。

2.  如果你的 ITSP 支持 (TLS) 的传输层安全，则 ITSP 必须为你提供证书。

<div>


> [!NOTE]  
> SIP 与实时传输协议协同工作 (RTP) 或安全实时传输协议 (SRTP) ，通过 Internet 协议管理实际语音数据的协议 (VoIP) 呼叫。
