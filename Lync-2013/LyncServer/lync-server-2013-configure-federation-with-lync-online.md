...
联合身份验证允许本地部署中的用户与你的组织中的 Microsoft 365 或 Office 365 用户进行通信。 若要配置联盟，请运行以下 cmdlet：
...
会话初始协议 (SIP) 地址是网络上每个用户的唯一标识符，类似于电话号码或电子邮件地址。 在尝试将 Lync 用户从本地移动到 Skype for business Online 之前，你需要将 Microsoft 365 或 Office 365 组织配置为与本地部署 (SIP) 地址空间共享共享会话初始协议。 如果未进行此配置，您可能会看到以下错误消息：
...
```
若要建立与 Skype for Business Online 的远程 PowerShell 会话，首先需要安装适用于 Windows PowerShell 的 Skype for Business Online 模块，可在此处下载： [https://go.microsoft.com/fwlink/p/?LinkId=391911](https://go.microsoft.com/fwlink/p/?linkid=391911) 。
